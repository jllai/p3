...
cur.execute('''
CREATE TABLE Tweets (tweet_id INTEGER PRIMARY KEY, text TEXT, user_posted INTEGER, time_posted DATETIME, retweets INTEGER, FOREIGN KEY (user_posted) REFERENCES Users(user_id))''')
...
for user in umich_tweets:
	...
	user_tup = (user['user']['id'],)
	cur.execute('SELECT user_id FROM Users WHERE user_id == (?)', user_tup)

	user_id_used = cur.fetchone()
	
	if user_id_used != user_tup:
	...
	for mentions in user['entities']['user_mentions']:
		user_tup = (mentions['id'],)
		cur.execute('SELECT user_id FROM Users WHERE user_id == (?)', user_tup)

		user_id_used = cur.fetchone()
		if user_id_used != user_tup:
			cur.execute('INSERT INTO Users (user_id, screen_name) VALUES (?, ?)', (mentions['id'], mentions['screen_name']))

# Add info to Tweets
for tweets in umich_tweets:
	tup = tweets['id'], tweets['text'], tweets['user']['id'], tweets['created_at'], tweets["retweet_count"]
